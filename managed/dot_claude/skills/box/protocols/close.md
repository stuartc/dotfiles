...
- `[terminal-state]` — optional, one of: `done` / `spun-out` / `abandoned` / `superseded-by <slug>`. If omitted, infer from the box state (all plan items done → `done`; everything reconciled to issues/new boxes with no plan items shipped → `spun-out`; abandoned mid-flight → `abandoned`) and confirm in one line before recording.
...
Done plan items are folded out of the active Next-moves view by the rollup in step 2; `close` doesn't re-archive them.
...
If anything was left deliberately open (a loose plan item, an unresolved question), name it in one line so the closure is honest.

...
Find the investigation root. Confirm `theories/T<id>-*/brief.md` exists. Read it.
...
Add the probe to the projected zone's `## Probes` table: number, theories tested, one-line purpose, pointer to the file. Don't list `.out` files separately — they're paired. This mirrors how `hypothesise` live-writes `## Theories`: the section is also regenerated by `rollup` from the `probes/` dir + probe-run logs, so a live edit here and a later rollup stay consistent.
